...
function buildUpstreamOAuthTestApp() {
  ...
  app.post("/token", (req, res) => {
    res.json({
      ...
      scope:
        "offline_access openid patient/Patient.read launch/patient veteran_status.read profile",
  ...
  app.use(function (req, res, next) {
    res
      .status(404)
      .send(`Upstream issuer test server did not recognize URL: ${req.url}`);
  });
...
module.exports = {
  start: () => {
    startServerInBackground(buildUpstreamOAuthTestApp(), UPSTREAM_OAUTH_PORT);
  },
