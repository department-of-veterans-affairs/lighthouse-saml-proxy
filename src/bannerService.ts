...
class RedisClientAdapter implements IRedisClient {
  ...
  async setAsync(key: string, value: string): Promise<void> {
    const setAsync = promisify(this.client.set).bind(this.client) as (
      key: string,
      value: string
    ) => Promise<void>;
  ...
  async getAsync(key: string): Promise<string | null> {
    const getAsync = promisify(this.client.get).bind(this.client) as (
      key: string
    ) => Promise<string | null>;
    return getAsync(key);
  }
  ...
  async delAsync(key: string): Promise<void> {
    const delAsync = promisify<string, number>(this.client.del).bind(
      this.client
    );
  ...
  async keysAsync(pattern: string): Promise<string[]> {
    const keysAsync = promisify(this.client.keys).bind(this.client) as (
      pattern: string
    ) => Promise<string[]>;
