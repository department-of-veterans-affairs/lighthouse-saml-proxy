...
import { buildBackgroundServerModule } from '../../oauth-proxy/tests/backgroundServer';
...
const userNotFoundTitle = 'Choose Login Provider';
...
function findAssertionInSamlResponse(samlResponse, assertion) {
  ...
  const attributes = parsed.getElementsByTagName('saml:Attribute');
  for(const element of attributes) {
    for(const inner of element.attributes) {
      if(inner.nodeValue === assertion) {
        return element;
      }
    }
  }
...
function responseResultType(response) {
  ...
  if(pageTitle(body) === userNotFoundTitle) {
...
function pageTitle(body) {
  ...
  const titles = parsed.getElementsByTagName('title');
